...
read -p "Suffix for conversation filenames, e.g. \" (WhatsApp)\" — blank for none []: " INBOX_SUFFIX
INBOX_SUFFIX="${INBOX_SUFFIX:-}"

# The whole point of the watchdog is telling you when reception dies. Without a
# channel it can only pop a local notification, which is useless if you are not
# at the machine.
echo
echo "Where should outage alerts go? (leave blank to decide later — doctor will remind you)"
read -p "  Email address for alerts []: " ALERT_EMAIL
ALERT_EMAIL="${ALERT_EMAIL:-}"
read -p "  Or a custom alert command (gets subject as \$1, body on stdin) []: " ALERT_COMMAND
ALERT_COMMAND="${ALERT_COMMAND:-}"
read -p "  Path to a node mail sender taking --to/--subject/--body (optional) []: " MAIL_SENDER
...
# The connector's operational state now lives INSIDE the user's vault: Signal
# identity keys, the message store, logs, and ~80 MB of node_modules. Many vaults
# are git repositories or sync to iCloud/Dropbox. Without an ignore file right
# here, the user's next `git add -A` commits the credential that authenticates
# their WhatsApp account. Write it before npm install creates the bulk.
cat > "$SCRIPTS_DIR/.gitignore" <<'IGNORE'
# Written by whatsapp-vault-connector's installer.
# This directory holds credentials and runtime state, never content.
# baileys_auth/ authenticates this machine AS your WhatsApp account —
# committing or syncing it is account takeover. Do not remove these lines.
baileys_auth/
baileys_auth_backup_*/
baileys_auth_pre_repair_*/
baileys_auth_sessions_backup_*/
baileys_store.json
.message_store.json
.message_store.json.tmp
.daemon_state.json
.daemon.lock
.daemon_heartbeat
.run/
...
.wa-alert-state
.wa-mute
.wa-watchdog-last-run
.wa-woke-at
wa-fix.log.jsonl
logs/
node_modules/
IGNORE
# Keep it out of iCloud Drive as well (Apple honours a .nosync suffix only on
# directories, so this is best-effort; the README explains the manual step).
touch "$SCRIPTS_DIR/.nosync" 2>/dev/null || true
ok "Wrote $SCRIPTS_DIR/.gitignore (keeps your WhatsApp keys out of your vault's git history)"
...
cmd_str = f"python3 '{wa_fix}' fix --quiet || true"
