...
# Read via plistlib, not PlistBuddy: PlistBuddy strips a leading space, and the
# conventional value is " (WhatsApp)". Losing that space would make the daemon
# write to a different filename than every existing conversation file, forking
# each thread in two.
WATCHDOG_PLIST="$HOME/Library/LaunchAgents/${LAUNCHD_LABEL_WATCHDOG}.plist"
_wd_env() { /usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:$1" "$WATCHDOG_PLIST" 2>/dev/null || echo ''; }
ALERT_EMAIL_EXISTING="$(_wd_env ALERT_EMAIL)"
ALERT_CMD_EXISTING="$(_wd_env WA_ALERT_COMMAND)"
MAIL_SENDER_EXISTING="$(_wd_env WA_MAIL_SENDER)"

SUFFIX_EXISTING="$(python3 -c "
import plistlib,sys
try:
    with open('$DAEMON_PLIST','rb') as f: d=plistlib.load(f)
    sys.stdout.write(d.get('EnvironmentVariables',{}).get('WA_INBOX_SUFFIX',''))
